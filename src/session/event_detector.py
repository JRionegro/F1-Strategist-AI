...
class RaceEventDetector:
    ...
    def _check_pit_window(
        ...
        try:
            ...
            for _, stint in driver_stints.iterrows():
                stint_start_raw = stint.get(
                    'StintStart', stint.get('LapStart'))
            ...
            stint_start_raw = current_stint.get(
                'StintStart', current_stint.get('LapStart', 1))
    ...
    def _check_safety_car(
        ...
        logger.debug(
            f"[SC_CHECK] Entering method with session_key={session_key}")
        try:
            race_control_df = self.provider.get_race_control_messages(
                session_key=session_key
            )
            logger.debug(
                f"[SC_CHECK] Got race_control_df, empty={
                    race_control_df.empty if race_control_df is not None else 'None'}")
            ...
            if time_col in race_control_df.columns:
                ...
                logger.debug(
                    f"[SC_CHECK] Filtered by {time_col} <= {current_time}")
            else:
                logger.warning(
                    f"[SC_CHECK] No time column found! Columns: {
                        list(
                            race_control_df.columns)}")
            ...
            logger.debug(
                f"[SC_CHECK] Found {
                    len(race_control_df)} messages before current time")
            ...
            for _, msg in recent.iterrows():
                ...
                msg_time = msg.get('Time') or msg.get(
                    'Date')  # Try both column names
                ...
                if any(kw in category or kw in message or kw in flag
                    ...
                    logger.info(
                        f"[SC_CHECK] Potential SC: Cat='{category}' Flag='{flag}' Msg='{message[:50]}'")
                ...
                if is_sc_ending:
                    ...
                    return RaceEvent(
                        event_type='safety_car_ending', priority=5, message=(
                            "✅ SAFETY CAR ENDING THIS LAP! "
                            "Prepare for restart. Manage tire temperature and gap."), data={
                            'category': category, 'message': message}, timestamp=current_time)
