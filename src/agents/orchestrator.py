...
class AgentOrchestrator:
    ...
    def set_context(self, context: AgentContext) -> None:
        ...
        logger.info(f"Context set for all agents: {context.session_type} - {context.race_name} {context.year}")
    
    async def query(self, query: str, context: Optional[AgentContext] = None) -> OrchestratedResponse:
        """
        ...
        logger.info(f"Routing query to {len(capable_agents)} agent(s): {capable_agents}")
        
        # Execute query on all capable agents
        responses = await self._execute_multi_agent_query(query, capable_agents)
        
        # Aggregate responses
        orchestrated = self._aggregate_responses(query, responses, capable_agents)
    ...
    def _build_multi_agent_response(
        ...
        combined = f"PRIMARY ANALYSIS ({primary.agent_name}):\n{primary.response}"
