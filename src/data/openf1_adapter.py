...
class SessionAdapter:
    ...
    def __init__(
        ...
        self.date = pd.to_datetime(
            date_start) if date_start else pd.Timestamp.now()
        ...
        logger.info(
            f"SessionAdapter created for {self.event_name} - {self.name}")
    ...
    def laps(self) -> pd.DataFrame:
        ...
        if self._laps is None:
            self.load_laps()
        # Type guard: after load_laps, _laps is always a DataFrame (possibly
        # empty)
    ...
    def results(self) -> pd.DataFrame:
        ...
        if self._results is None:
            self.load_results()
        # Type guard: after load_results, _results is always a DataFrame
        # (possibly empty)
    ...
    def load(
        ...
        """
        logger.info(
            f"Loading session data for {self.event_name} - {self.name}")
    ...
    def load_laps(self) -> None:
        ...
        if "LapTime_seconds" in laps_df.columns:
            laps_df["LapTime"] = pd.to_timedelta(
                laps_df["LapTime_seconds"], unit="s")
        ...
        if "LapStartTime" in laps_df.columns and not laps_df.empty:
            ...
            if "LapTime" in laps_df.columns:
                laps_df["LapEndTime"] = laps_df["LapStartTime"] + \
                    laps_df["LapTime"]
...
def get_session(
    ...
    logger.info(
        f"Fetching session: {year} Round {round_number} {session_name}")
    ...
    if not session_info:
        logger.error(
            f"Session not found: {year} Round {round_number} {session_name}")
        return None
