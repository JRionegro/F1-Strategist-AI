...
def _resolve_session_key(
    ...
    *,
    ...
    if session_key is not None:
        session = provider.get_session(year=year, round_number=None, session_name="Race")
        return int(session_key), session or {}

    target_date = _parse_race_date(race_name)
    sessions = provider._request("sessions", {"year": year, "session_name": "Race"})
    if not sessions:
...
def _build_driver_map(drivers_df: pd.DataFrame) -> Dict[str, str]:
    ...
    for _, row in drivers_df.iterrows():
        ...
        abbr = str(row.get("Abbreviation")) if row.get("Abbreviation") else number
        mapping[number] = abbr
    return mapping


def _apply_stints(laps_df: pd.DataFrame, stints_df: pd.DataFrame) -> pd.DataFrame:
...
def _normalize_laps(
    ...
    frame["driver_id"] = frame["DriverNumber"].astype(str).map(lambda x: driver_map.get(str(x), str(x)))
...
def _normalize_pits(pit_df: pd.DataFrame, driver_map: Dict[str, str]) -> pd.DataFrame:
...
def fetch_and_cache_openf1_race(
    *,
    ...
    provider = OpenF1DataProvider()
    resolved_session_key, _ = _resolve_session_key(provider, year=year, race_name=race_name, session_key=session_key)
    ...
    lap_saved = cache_manager.save_race_data(year, race_name, DataType.LAP_TIMES, normalized_laps)
    pit_saved = cache_manager.save_race_data(year, race_name, DataType.PIT_STOPS, normalized_pits)
    ...
    lap_path = cache_manager._get_cache_file_path(year, race_name, DataType.LAP_TIMES)
    pit_path = cache_manager._get_cache_file_path(year, race_name, DataType.PIT_STOPS)
